...
$KW.IGallery = (function() {
    ...
    var module = {
        ...
        render: function(galleryModel, context) {
            ...
            var view = galleryModel.viewtype;
            var htmlString = "";
            if(view == "pageview") {
                htmlString = "<div" + html + "name='touchcontainer_HStrip' class='" + computedSkin + " kstripcontainer' style='" + marginPaddding + "'>";
                htmlString += "<div id='imgs' class='kstrip' style='" + (!kony.appinit.useTransition ? "position:relative" : "") + "'>";
                htmlString += "</div></div>";

                if(!kony.appinit.useTransition)
                    htmlString += "<div class='ktable kwt100' id='" + galleryModel.pf + "_" + galleryModel.id + "_footer'></div>";
            } else {
                htmlString = "<div" + html + "class='" + computedSkin + "' style='" + marginPaddding + "'>";
