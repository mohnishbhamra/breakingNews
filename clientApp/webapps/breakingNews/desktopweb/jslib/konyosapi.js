...
$KI.os.print = function(container, autoClose) {
    $KU.logExecuting('kony.os.print');
    $KU.logExecutingWithParams('kony.os.print', container, container);
    if(container && container.id && !(container.wType === "Form")) {
        var domNode = document.getElementById(container.id);
        if(container.wType && container.wType === "Popup")
            domNode = document.getElementById(container.id + "_body");
        if(!domNode)
            domNode = $KU.getNodeByModel(container);
        if(domNode) {
            var popupHeight;
            if(container.wType && container.wType === "Popup") {
                var popupContainer = document.getElementById(container.id + "_container");
                if(popupContainer && popupContainer.className)
                    domNode.className = popupContainer.className
                popupHeight = domNode.style.height;
                domNode.style.height = "auto";
            }
            var printContents = domNode.outerHTML;
            if(popupHeight !== 'undefined')
                domNode.style.height = popupHeight;
    ...
    $KU.logExecutingFinished('kony.os.print');
};
...
$KI.os.addbookmark = function(url, title, params) {
    ...
    try {
        ...
        } else if(kony.appinit.isIE && typeof window.external == "object") {
            window.external.AddFavorite(url, title); 
        } else if(kony.appinit.isOpera) {
...
$KIO.FileSystem = $KIO.fs = {
    ...
    init: function() {
        if(!$KU.isAjaxUploadSupported) {
            var htmlString = "<iframe id='konyiframeElem' name='konyiframeElem' src='about:blank' style='opacity:0;width:0px;height:0px;' ></iframe>";
    ...
    browse: function(browseConfig, browseCallback) { 
        ...
        if($KU.isAjaxUploadSupported) {
            ...
            if(!fileElement) {
                htmlString = "<form name='uploadForm'><input type='file' multiple id='konyFileElem' onchange='$KIO.fs.afterBrowse(arguments[0])' onclick='uploadForm.reset();' style='opacity:0;width:0px;height:0px;display:none;' /></form>";
                var wrapper = document.createElement('div');
                wrapper.innerHTML = htmlString;
                document.body.appendChild(wrapper);
            ...
            var iFrameWindow = document.getElementById("konyiframeElem").contentWindow || document.getElementById("konyiframeElem").documentWindow;
    ...
    uploadFiles: function(URL, fileList, upLoadCallBack, upLoadConfig) { 
        ...
        if($KU.isAjaxUploadSupported) {
            ...
            var konyFrame = document.getElementById("konyiframeElem");
            if($KU.isIE) {
                konyFrame.attachEvent("onload", $KIO.fs.ieCallback);
                konyFrame.attachEvent("onerror", $KIO.fs.ieErrorCallback);
            } else {
                konyFrame.onload = $KIO.fs.ieCallback;
                konyFrame.onerror = $KIO.fs.ieErrorCallback;
            }
            ...
            var frame = document.getElementById('konyiframeElem');
    ...
    ieCallback: function() {
        ...
        try {
            var response = document.getElementById("konyiframeElem").contentWindow.document.body.innerHTML;
            if(response) {
