...
$KW.Calendar = (function() {
    ...
    var _generateInputTagString = function(calendarModel, context) {
        ...
        inputTagString += (!(calendarModel.dateEditable || calendarModel.dateeditable) ? " readonly='readonly'" : "");
    ...
    var _show = function(calendarModel, targetCalendar, ms, flag) {
        ...
        mouseEvent = kony.appinit.isFirefox ? "DOMMouseScroll" : "mousewheel";
        kony.events.addEventListener(mainContainer, mouseEvent, _dismissCalendar);
        kony.events.addEventListener(window, 'scroll', _dismissCalendar);
    };
    ...
    var module = {
        ...
        calendarTextfieldChangeEventHandler: function(eventObject, target) {
            var calendarNode = target.parentNode,
                ...
                dateValue, currentDate = new Date(),
                validStartDateSet, validEndDateSet, calenEventref, resultString, result;

            if(target.getAttribute("kmasterid")) {
                calendarModel = $KU.getWidgetModelByID(target.getAttribute("kformname") + "_" + $KU.getElementID(calendarId));
            } else {
                calendarId = calendarId.split("_").length > 2 ? calendarId.substring(0, calendarId.lastIndexOf("_")) : calendarId;
                calendarModel = kony.model.getWidgetModel(target.getAttribute("kformname"), $KU.getElementID(calendarId), target.getAttribute("ktabpaneid"));
            }
            if(containerId) {
            ...
            if(dateValue.length > 0 && calendarModel.dateComponents) {
                if(result != false) {
                    calendarModel.dateComponents = result.date;
                    target.className = target.className.replace('konyplaceholder', "");
        ...
        destroyCalendar: function(ele, calendarElementID, targetWidget) {
            ...
            mouseEvent = kony.appinit.isFirefox ? "DOMMouseScroll" : "mousewheel";
            kony.events.removeEventListener(main, mouseEvent, _dismissCalendar);
            kony.events.removeEventListener(window, 'scroll', _dismissCalendar);
        },
