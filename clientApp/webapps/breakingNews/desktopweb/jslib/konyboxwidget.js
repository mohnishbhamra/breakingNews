...
$KW.HBox = (function() {
    ...
    var module = {
        ...
        render: function(boxModel, context) {
            ...
            if(boxModel.layouttype == constants.CONTAINER_LAYOUT_GRID) {
                return $KW.Grid.render(boxModel, context);
            }

            if(boxModel.isTemplate === true) {
                var parentModel = boxModel;
            } else if(!context.renderingTabHeader) {
        ...
        addChild: function(boxModel, wArray) {
            ...
            if(box && wArray.length > 0) {
                ...
                if(boxModel.wType == "ScrollBox" && boxModel.orientation == constants.BOX_LAYOUT_HORIZONTAL) {
                    var scrollNode = box;
                    $KW.ScrollBox.adjustScrollBoxWidth(boxModel, scrollNode);
                    box = box.firstChild.firstChild;
                } else if(boxModel.wType == "ScrollBox" && boxModel.orientation == constants.BOX_LAYOUT_VERTICAL) {
                    box = box.firstChild.firstChild; 
                }
                ...
                if(boxModel.wType == "ScrollBox" && boxModel.orientation == constants.BOX_LAYOUT_HORIZONTAL) {
                    $KW.ScrollBox.adjustScrollChildrenWidth(boxModel, scrollNode);
                }
        ...
        addChildat: function(boxModel, wArray, index) {
            ...
            if(box) {
                ...
                if(kony.appinit.isIE && !box.childNodes[index]) {
                    box.appendChild(wrapper.childNodes[0]);
                } else
        ...
        DOMremove: function(boxModel, widgetref) {
            var box = $KU.getNodeByModel(boxModel);
            if(box && widgetref) {
                if(boxModel.wType == "ScrollBox" && (boxModel.orientation == constants.BOX_LAYOUT_HORIZONTAL || boxModel.orientation == constants.BOX_LAYOUT_VERTICAL)) {
                    box = box.firstChild.firstChild;
                } else if(boxModel.wType == "HBox" || boxModel.orientation == constants.BOX_LAYOUT_HORIZONTAL) {
                    box = boxModel.wType == "ScrollBox" ? box.firstChild.firstChild : box;
                }
                for(var i = 0; i < boxModel.children.length; i++) {
