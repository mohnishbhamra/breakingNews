...
function ieSetProperty(obj, key, value) {
    kony.model.update(obj, key, value);
}
...
$KU = kony.utils = {
    ...
    setDisabledProperty: function(node, konyAttribute, htmlAttribute, value, widgetModel) {
        ...
        if(widgettype == "ComboBox" && node.childNodes[0] && node.childNodes[0].tagName == "INPUT") {
            if(value || removeDisabled) {
                node.childNodes[0].disabled = value;
            }
        }

        
        if((widgettype == "RadioButtonGroup" || widgettype == "CheckBoxGroup") && node.tagName == "INPUT" && widgetModel.itemorientation == "vertical") {
            node.parentNode[attributeFunction](konyAttribute, value);
        }
    ...
    getModelByNode: function(target) {
        ...
        return kony.model.getWidgetModel(formID, widgetID, tabPaneID, tabID);
    },
    ...
    setorientationDelay: function() {
        var delay = 100;
        if(kony.appinit.isIDevice && kony.appinit.isSafari) {
            delay = 0;
        }
    ...
    imgLoadHandler: function(event, img) {
        ...
        if(tabID) {
            widgetModel = kony.model.getWidgetModel(img.getAttribute("kformname"), $KU.getElementID(targetWidgetID), tabPaneID, tabID);
        } else {
    ...
    onImageLoadComplete: function(widgetModel, img) {
        ...
        if(containerModel) {
            ...
            if(topLevelModel && topLevelModel.wType == 'Popup') {
                ...
                if(popupConatiner) {
                    var popupElem = topLevelModel.ismodal ? popupConatiner.getElementsByTagName('form')[0] : popupConatiner;
                    $KW.Popup.adjustPopupDimensions(topLevelModel, popupElem);
                }
            }
            if(containerModel.wType == 'MenuContainer' && containerModel.viewtype == constants.MENU_CONTAINER_VIEW_TYPE_DROPDOWNVIEW) {
                var menuItem = $KU.getParentByAttribute(img, "menuindex");
                if(menuItem) {
                    var index = menuItem.getAttribute("menuindex");
    ...
    isTabIndexRequired: function(widgetModel) {
        if(["Image", "Switch", "Link", "Label", "RichText"].indexOf(widgetModel.wType) >= 0) {
            return true;
        }
