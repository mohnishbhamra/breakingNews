...
$KW.Form = (function() {
    ...
    var module = {
        initialize: function() {
            kony.events.addEvent("onresize", "Form", this.resizeHandler);
        ...
        resizeHandler: function(formModel, width) {
            var frameworkReturnValue = false,
                resizeReturnValue = true,
                resizeEventRef = null,
                breakpointChangeReturnValue = false;

            if($KG.appbehaviors["responsive"] !== true) return;

            if(module.breakpointChanged(formModel, width)) {
                frameworkEventRef = $KU.returnEventReference(formModel.onbreakpointhandler);
                breakpointEventRef = $KU.returnEventReference(formModel.onbreakpointchange);

                if(frameworkEventRef) {
                    frameworkReturnValue = $KU.executeWidgetEventHandler(formModel, frameworkEventRef, $KG["__currentFormBreakpoint"]);
                }
                if(breakpointEventRef) {
                    breakpointChangeReturnValue = $KU.executeWidgetEventHandler(formModel, breakpointEventRef, $KG["__currentFormBreakpoint"]);
                }
            }

            resizeEventRef = $KU.returnEventReference(formModel.onresize);
            if(resizeEventRef) {
                resizeReturnValue = $KU.executeWidgetEventHandler(formModel, resizeEventRef, width);
            }

            if(frameworkReturnValue !== false) {
                frameworkReturnValue = true;
            }
            if(breakpointChangeReturnValue !== false) {
                breakpointChangeReturnValue = true;
            }
            if(resizeReturnValue !== false) {
                resizeReturnValue = true;
            }

            if(frameworkReturnValue ||
                breakpointChangeReturnValue ||
                resizeReturnValue) {
        ...
        addChild: function(formModel, wArray, bVisibility) {
            if(($KG["__currentForm"] && formModel.id == $KG["__currentForm"].id) || formModel.wType == 'Popup') {
                ...
                if(formModel.wType == 'Popup')
                    formNode = $KU.getElementById(formModel.id + "_body").children[0];
        ...
        addChildAt: function(formModel, widget, index) {
            if(($KG["__currentForm"] && formModel.id == $KG["__currentForm"].id) || formModel.wType == 'Popup') {
                ...
                if(formModel.wType == 'Popup' && $KU.getElementById(formModel.id + "_body"))
                    formNode = $KU.getElementById(formModel.id + "_body").children[0];
        ...
        generateTemplate: function(headerID, type, context) {
            var headerModel = kony.model.getWidgetModel(headerID);
            var htmlString = "";
                    ...
                    if(!rendered) {
                        ...
                        if(prevForm) {
                            if(prevForm.retainscrollposition) {
                                prevForm.y = window.pageYOffset || document.documentElement.scrollTop;
                            }
                            module.destroyTouchWidgets(prevForm.id);
                        }
        ...
        applyTransition: function(previousForm, currentForm) {
            ...
            if(!$KG["disableTransition"] && (inTrans || outTrans)) {
                ev = (kony.appinit.isFirefox || kony.appinit.isIE11) ? "animationend" : $KU.animationEnd;
