...
$KW.DataGrid = (function() {
    ...
    var module = {
        ...
        create: function(gridModel, context) {
            ...
            if((gridModel.enablescrollbar == 1 && !gridModel.dockingheader) || $KW.FlexUtils.isFlexWidget(gridModel)) {
                ...
                htmlString += "<div class='" + computedSkin + "' style='overflow-y: auto; overflow-x: hidden;border-collapse:collapse;" + margin + "'>"
                ...
                htmlString += "<table style='" + wstyle + margin + "' class='" + computedSkin + "' " + $KW.Utils.getBaseHtml(gridModel, gridModel.context) + ">";
            }
        ...
        headerBuilder: function(columnModel, gridModel) {
            ...
            for(var i = IndexJL; i < columnModel.length; i++) {
                ...
                if(headerTemp) {
                    ...
                    if(boxModel) {
                        gridModel.counter = 0;
                        temp += $KW.Utils.handleLayout(gridModel, boxModel, rowData);
                    }
        ...
        updateData: function(childModel, childNode, gridModel, row, canExecute) {
            if(gridModel && row) {
                ...
                if(canExecute) {
                    var eventExecuted = kony.events.executeBoxEvent(childModel, item, gridModel, "", index);
                    if(!eventExecuted) {
                                ...
                                if(htmlRow) {
                                    if(action == "applycellskin") {
                                        ...
                                        if(template) {
                                            ...
                                            htmlRow.cells[cellIndex].innerHTML = $KW.Utils.handleLayout(gridModel, template, dataArray);
                                        } else
    ...
    module.ColumnResize = function(grid, gridModel) {
        ...
        this.t.cs = parseInt(kony.appinit.isIE ? grid.cellSpacing || grid.currentStyle.borderSpacing : $KU.getStyle(grid, "border-spacing")) || 2;
        this.t.b = parseInt(kony.appinit.isIE ? grid.border || grid.currentStyle.borderLeftWidth : $KU.getStyle(grid, 'border-left-width')) || 1; 
        ...
        this.cGrips.style.margin = $KU.joinArray(gridModel.margin, "% ") + "%";
    ...
    module.ColumnResize.prototype = {
        init: function() {
            ...
            for(var i = 0; i < this.cGrips.children.length; i++) {
                ...
                if(i < this.t.ln - 1) kony.events.addEventListener(this.cGrips.childNodes[i], "mousedown", this.handleEventListener);
            }
